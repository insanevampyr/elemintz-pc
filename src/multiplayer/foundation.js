...
export function createMultiplayerFoundation({
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("profile:getOrCreateReferralCode", async (payload = {}, respond = () => {}) => {
      respond = toAckCallback(respond);
      const sessionResult = await ensureSocketSession(socket, payload, { allowBootstrap: false });
      if (!sessionResult?.ok) {
        respond(sessionResult);
      ...
      if (!sessionResult.session?.authenticated || typeof accountStore?.getOrCreateReferralCode !== "function") {
        respond(buildAccountError({
          code: "AUTH_REQUIRED",
          message: "An authenticated EleMintz account session is required."
        }));
    ...
    socket.on("admin:listEventChestDrafts", async (payload = {}, respond = () => {}) => {
      ...
      try {
        assertAdminAccessForSession(sessionResult.session);
        if (typeof profileAuthority?.listEventChestDraftsForAdmin !== "function") {
          throw Object.assign(new Error("Event Chest draft authority is not available."), {
            code: "EVENT_CHEST_DRAFT_AUTHORITY_UNAVAILABLE"
          });
        }
        const drafts = await profileAuthority.listEventChestDraftsForAdmin();
        respond({
          ok: true,
          result: {
            drafts
        ...
        respond(buildAdminError(error, "EVENT_CHEST_DRAFT_LIST_FAILED"));
      }
    });

    socket.on("admin:getEventChestDraft", async (payload = {}, respond = () => {}) => {
      respond = toAckCallback(respond);
      const sessionResult = await ensureAdminSession(socket, payload);
      if (!sessionResult?.ok) {
        respond(buildAdminError(sessionResult?.error, "ADMIN_AUTH_REQUIRED"));
      ...
      try {
        assertAdminAccessForSession(sessionResult.session);
        if (typeof profileAuthority?.getEventChestDraftForAdmin !== "function") {
          throw Object.assign(new Error("Event Chest draft authority is not available."), {
            code: "EVENT_CHEST_DRAFT_AUTHORITY_UNAVAILABLE"
          });
        }
        const draft = await profileAuthority.getEventChestDraftForAdmin(payload?.draftId);
        respond({
          ok: true,
          result: {
            draft
        ...
        respond(buildAdminError(error, "EVENT_CHEST_DRAFT_GET_FAILED"));
      }
    });
    ...
    socket.on("admin:validateEventChestDraft", async (payload = {}, respond = () => {}) => {
      ...
      try {
        assertAdminAccessForSession(sessionResult.session);
        if (typeof profileAuthority?.validateEventChestDraftForAdmin !== "function") {
          throw Object.assign(new Error("Event Chest draft authority is not available."), {
            code: "EVENT_CHEST_DRAFT_AUTHORITY_UNAVAILABLE"
          });
        }
        const result = await profileAuthority.validateEventChestDraftForAdmin({
        ...
        respond(buildAdminError(error, "EVENT_CHEST_DRAFT_VALIDATE_FAILED"));
      }
    });
    ...
    socket.on("admin:saveEventChestDraft", async (payload = {}, respond = () => {}) => {
      ...
      try {
        const adminAccess = assertAdminAccessForSession(sessionResult.session);
        if (typeof profileAuthority?.saveEventChestDraftForAdmin !== "function") {
          throw Object.assign(new Error("Event Chest draft authority is not available."), {
            code: "EVENT_CHEST_DRAFT_AUTHORITY_UNAVAILABLE"
          });
        }
    ...
    socket.on("profile:claimCollectionAlbumReward", async (payload = {}, respond = () => {}) => {
      respond = toAckCallback(respond);
      const sessionResult = await ensureClaimedProfileAccess(socket, payload, {
        allowBootstrap: true
      });
      ...
      if (typeof profileAuthority?.claimCollectionAlbumReward !== "function") {
        respond({
          ok: false,
          error: {
            code: "PROFILE_AUTHORITY_UNAVAILABLE",
      ...
      try {
        const result = await profileAuthority.claimCollectionAlbumReward({
          ...payload,
          username: sessionResult.session?.profileKey ?? sessionResult.session?.username
        });
        respond({
        ...
        respond({
          ok: false,
          error: {
            code: "COLLECTION_ALBUM_REWARD_FAILED",
            message: String(error?.message ?? "Unable to claim Collection Album reward.")
          }
        });
      }
    });

    socket.on("profile:addPlayTime", async (payload = {}, respond = () => {}) => {
      respond = toAckCallback(respond);
      const sessionResult = await ensureClaimedProfileAccess(socket, payload, {
        allowBootstrap: true
      });
      ...
      if (typeof profileAuthority?.addProfilePlayTime !== "function") {
        respond({
          ok: false,
          error: {
            code: "PROFILE_AUTHORITY_UNAVAILABLE",
      ...
      try {
        const result = await profileAuthority.addProfilePlayTime({
          ...payload,
          username: sessionResult.session?.profileKey ?? sessionResult.session?.username
        });
        respond({
        ...
        respond({
          ok: false,
          error: {
            code: "PROFILE_PLAY_TIME_UPDATE_FAILED",
            message: String(error?.message ?? "Unable to update profile time played.")
          }
        });
      }
    });

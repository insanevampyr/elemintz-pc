...
export function registerMultiplayerIpcHandlers(ipcMain, options = {}) {
  ...
  ipcMain.handle("multiplayer:getReferralDashboard", async (event, payload) => {
    subscribers.add(event.sender);
    return client.getReferralDashboard(payload);
  });

  ipcMain.handle("multiplayer:claimReferralReward", async (event, payload) => {
    subscribers.add(event.sender);
    return client.claimReferralReward(payload);
  });

  ipcMain.handle("multiplayer:activateReferralCode", async (event, payload) => {
    subscribers.add(event.sender);
    return client.activateReferralCode(payload);
  });
  ...
  ipcMain.handle("multiplayer:openEventChestEntitlement", async (event, payload) => {
    subscribers.add(event.sender);
    const entitlementId = typeof payload === "string" ? payload : payload?.entitlementId;
  ...
  ipcMain.handle("multiplayer:addProfilePlayTime", async (event, payload) => {
    subscribers.add(event.sender);
    return client.addProfilePlayTime(payload);
  });

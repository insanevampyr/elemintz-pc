...
test("ui: shared battle status summary preserves standard order, fallbacks, and optional details", () => {
  const html = renderBattleStatusSummary({
    round: null,
    primaryCardsTaken: { count: 3 },
    secondaryCardsTaken: { role: "rival", count: undefined },
    warCount: null,
    detailLines: [
  ...
  assert.match(html, /Cards Taken:<\/span>\s*<span class="battle-status-value">You 3 · Rival —/);
...
test("ui: game screen renders active Gauntlet streak and rival details when present", () => {
  ...
  assert.match(html, /Cards Taken:<\/span>\s*<span class="battle-status-value">You 0 · Rival 0/);
  assert.match(html, /Mode:<\/span>\s*<span class="battle-status-value">Gauntlet/);
...
test("ui: game screen uses provided variant card images", () => {
  ...
  assert.match(html, /Cards Taken:<\/span>\s*<span class="battle-status-value">You 0 · Opponent 0/);
...
test("ui: hidden hand remains hidden while WAR pile is face-up", () => {
  ...
  assert.match(html, /Cards Taken:<\/span>\s*<span class="battle-status-value">P1 0 · P2 0/);
  assert.match(html, /Mode:<\/span>\s*<span class="battle-status-value">Local Hotseat/);
  assert.doesNotMatch(html, /Captured: Player 1 • 0 \| Player 2 • 0/);
});
...
test("ui: local PvE active WAR center result prefers the latest clash cards over stale lastRound data", () => {
  ...
  assert.match(html, /Cards Taken:<\/span>\s*<span class="battle-status-value">You 3 · Opponent 0/);
...
test("ui: online play screen renders server-authoritative timers above the board and in the topbar", () => {
  const html = onlinePlayScreen.render({
    backgroundImage: "assets/EleMintzIcon.png",
    joinCode: "ABC123",
    onlineMatchTimer: {
  ...
  const onlineTopbarStart = html.indexOf('class="screen-topbar online-play-topbar"');
  const themedSurfaceStart = html.indexOf('class="arena-board screen-themed-surface"');
  const onlineRightStatusStart = html.indexOf('data-online-status-zone="right"');
  assert.ok(onlineTopbarStart >= 0);
  assert.ok(html.indexOf('data-online-turn-timer-shell="true"', onlineTopbarStart) < themedSurfaceStart);
  assert.doesNotMatch(html.slice(onlineRightStatusStart), /data-battle-status-detail="online-turn-timer"/);
  assert.doesNotMatch(html.slice(onlineRightStatusStart), /Timer:<\/span>/);
  assert.match(html, /data-online-active-meta-strip="true"/);
});
...
test("ui: easy PvE match complete payload shows practice-mode reward suppression clarity", () => {
  ...
  const payload = controller.buildMatchCompleteModalPayload(
    "pve",
    {
      ...
      difficulty: "easy",
      history: [
        { result: "p1", warClashes: 1, capturedCards: 4, capturedOpponentCards: 2, p1Card: "fire", p2Card: "fire" },
        { result: "p2", warClashes: 4, capturedCards: 0, capturedOpponentCards: 0, p1Card: "earth", p2Card: "fire" }
      ],
      players: {
        p1: { hand: [] },
        p2: { hand: ["fire", "water"] }
  ...
  assert.match(payload.bodyHtml, /<span class="match-complete-stat-label">WARs Entered<\/span>\s*<strong class="match-complete-stat-value">2<\/strong>/);
  assert.match(payload.bodyHtml, /<span class="match-complete-stat-label">Longest WAR<\/span>\s*<strong class="match-complete-stat-value">4<\/strong>/);
...
test("ui: tracked PvE match complete payload still prefers persisted WAR stats over match-derived values", () => {
  ...
  const payload = controller.buildMatchCompleteModalPayload(
    "pve",
    {
      ...
      difficulty: "hard",
      history: [
        { result: "p1", warClashes: 1, capturedCards: 4, capturedOpponentCards: 2, p1Card: "fire", p2Card: "water" },
        { result: "p1", warClashes: 4, capturedCards: 8, capturedOpponentCards: 4, p1Card: "earth", p2Card: "fire" }
      ],
      players: {
        p1: { hand: ["earth"] },
        p2: { hand: [] }
...
test("ui: tracked PvE match complete payload renders corrected persisted WAR stats for multiple chains", () => {
  const controller = createRendererController();
  controller.username = "TrackedWarSummaryUser";
  controller.profile = { username: "TrackedWarSummaryUser" };
  controller.gameController = { captured: { p1: 0, p2: 0 } };
  ...
  const payload = controller.buildMatchCompleteModalPayload(
    "pve",
    {
      ...
      difficulty: "normal",
      history: [
        { result: "p2", warClashes: 1, capturedCards: 4, capturedOpponentCards: 2, p1Card: "fire", p2Card: "water" },
        { result: "p1", warClashes: 4, capturedCards: 8, capturedOpponentCards: 4, p1Card: "earth", p2Card: "fire" }
      ],
      players: {
        p1: { hand: [] },
